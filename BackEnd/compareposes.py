...
def closeness_of_pose(ref, mannequin):
    ...
    if preference == "Upper":
        closeness = (headdist+leftShoulderdist+rightShoulderdist+leftElbowdist+rightElbowdist+leftWristdist+rightWristdist)/7
    if preference == "Lower":
         closeness = (leftHipdist+rightHipdist+leftKneedist+rightKneedist+leftAnkledist+rightAnkledist)/6
...
poses = []
with open(working_dir + poses_path, 'r') as input:
